/*
 * Cockpit Agent - Main Application
 *
...
export const Application = () => {
  ...
  const [detectedSecrets, setDetectedSecrets] = useState<
    { id: string; type: string; detectedAt: Date }[]
  >([]);
  ...
  const [sessionSaveTimeout, setSessionSaveTimeout] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);

  const activeSessionId = currentSession?.id ?? null;
  const terminalReady = activeSessionId
    ? Boolean(terminalReadyBySession[activeSessionId])
    : false;
  ...
  const waitForTerminalReady = useCallback(
    (sessionId: string, timeoutMs: number = 10000): Promise<boolean> => {
  ...
  const getOrCreateAgent = useCallback(
    (sessionId: string): { agent: AgentController; created: boolean } => {
  ...
  useEffect(() => {
    ...
    hostnameFile.watch((content) =>
      setHostname((content as string)?.trim() ?? "unknown"),
    );
  ...
  const handleSendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        const getActionKey = (
          action: PendingAction | Message["action"] | null | undefined,
        ) => {
          if (!action) return "";
          switch (action.type) {
            case "command":
        ...
        const response = await activeAgent.processMessage(content, {
          ...
          onActionExecuted: (action, result) => {
            ...
            setMessages((prev) => {
              ...
              const runningActionIdx = prev.findIndex(
                (m) =>
                  m.role === "action" &&
                  !m.result &&
        ...
        if (streamingMessageId) {
          ...
          !intermediateResponseShown ||
          response !== lastIntermediateResponse
        ) {
          const assistantMessage: Message = {
            id: `${Date.now()}-${Math.random().toString(36).slice(2)}`,
            role: "assistant",
  ...
  const handleChangeSafetyMode = useCallback(
    (mode: string) => {
      ...
      <div className="ai-agent-header">
        ...
        >
          <FlexItem>
            ...
            >
              <FlexItem>
                <img src={logoUrl} alt="Logo" className="header-icon-img" />
              </FlexItem>
              <FlexItem>
                <span className="header-title">{_("Agent")}</span>
              </FlexItem>
              <FlexItem>
                <div className="header-status-pill">
                  <span>{settings.model}</span>
                  <span style={{ opacity: 0.5, margin: "0 4px" }}>@</span>
                  <span>{hostname || _("loading...")}</span>
                </div>
              </FlexItem>
              {(() => {
            ...
            <Flex spaceItems={{ default: "spaceItemsSm" }}>
              ...
              {settings.debugMode && (
                <FlexItem>
                  <Button
                    variant="plain"
                    aria-label={_("Toggle Debug Panel")}
