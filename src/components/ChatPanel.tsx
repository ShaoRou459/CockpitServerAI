            ...
            <div className="chat-empty-state">
              ...
              <div className="chat-quick-actions">
                <button
                  ...
                  onClick={() => onSendMessage("Check system storage")}
                >
                  {_("Check system storage")}
                </button>
                <button
                  type="button"
                  className="chat-quick-action-btn"
                  onClick={() => onSendMessage("Update Docker containers")}
                >
                  {_("Update Docker containers")}
                </button>
                <button
                  type="button"
                  className="chat-quick-action-btn"
                  onClick={() => onSendMessage("Show network info")}
                >
                  {_("Network info")}
                </button>
                <button
                  type="button"
                  className="chat-quick-action-btn"
                  onClick={() => onSendMessage("Check security logs")}
                >
                  {_("Check security logs")}
                </button>
              </div>
            </div>
          )}
    ...
    <div className="file-content-collapsible">
      ...
      >
        ...
        <span className="file-content-toggle-meta">
          {lineCount} {lineCount === 1 ? _("line") : _("lines")} • {charCount}{" "}
          {_("chars")}
        </span>
      </button>
      {expanded && (
...
const InlineApproval: React.FC<{
  ...
  return (
    <div
      className={`approval-compact risk-${action.risk_level}`}
      style={{ borderLeftColor: getRiskColor() }}
    >
      <div className="approval-compact-header">
        ...
        >
          ...
          <span
            className="approval-compact-risk"
      ...
      <div
        className={`action-compact-details ${expanded ? "action-compact-details--open" : ""}`}
      >
        <div className="action-compact-description">{action.description}</div>
        {action.type === "file_write" && action.content && (
          <FileContentCollapsible
            label={_("Content to Write")}
            content={action.content}
          />
        )}
        {action.risk_level === "critical" && (
          <div className="approval-compact-warning">
            {_("Critical operation - review carefully")}
          </div>
        )}
      </div>
    </div>
      ...
      >
        ...
        {state === "running" && (
          <div className="action-compact-running">
            {_("Running... (watch the terminal for live output)")}
          </div>
        )}
        {/* Command output */}
  ...
  return (
    <div className="interactive-notice">
      <div className="interactive-notice-header">
        ...
        <span className="interactive-notice-title">
          {_("Interactive Command")}
        </span>
      </div>
      {commandText && (
      ...
      <div className="interactive-notice-hint">{message.content}</div>
    </div>
  );
};
...
const MessageBubble: React.FC<{ message: Message }> = ({ message }) => {
  ...
  const renderContent = () => {
    if (isUser) {
      ...
      let cleanContent = rawJsonContent
