...
export const SecretsIndicator: React.FC<SecretsIndicatorProps> = ({
    ...
    const getTypeName = (type: string) => SECRET_TYPE_NAMES[type] || type;

    const popoverContent = (
        <div style={{ minWidth: '280px', maxWidth: '400px' }}>
            <div style={{ marginBottom: '12px' }}>
                <Content component={ContentVariants.h4} style={{ margin: 0, display: 'flex', alignItems: 'center' }}>
                    <ShieldAltIcon style={{ marginRight: '8px' }} />
                    {_("Detected Secrets")}
                </Content>
                <Content component={ContentVariants.small} style={{ color: '#6a6e73' }}>
                    {_("These values are hidden from the AI but can be used in commands via their placeholder IDs.")}
                </Content>
            </div>
            ...
            {secretCount === 0 ? (
                <EmptyState>
                    <EmptyStateBody>
                        {_("No secrets detected yet. Sensitive data like passwords and API keys will appear here.")}
                    </EmptyStateBody>
                </EmptyState>
            ) : (
                <>
    ...
    return (
        ...
        >
            <Button
                ...
                aria-label={`${secretCount} secrets detected`}
            >
                <ShieldAltIcon />
                {secretCount > 0 && (
