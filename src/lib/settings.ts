...
export async function loadSettings(): Promise<Settings> {
  try {
    ...
    if (content && typeof content === "string") {
      ...
      if (
        decryptedApiKey &&
        (parsed._encrypted || isEncrypted(decryptedApiKey))
      ) {
        try {
        ...
        console.log(
          "Found legacy plaintext API key, will encrypt on next save",
        );
      }
  ...
  try {
    ...
    if (stored) {
      ...
      console.log(
        "Migrated settings from localStorage to server (with encryption)",
      );
      return settings;
    }
  } catch (e) {
